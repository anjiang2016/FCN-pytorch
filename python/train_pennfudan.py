...
dataset = PennFudanDataset('/Users/zhaomingming/data_sets/PennFudanPed', get_transform(train=True))
dataset_test = PennFudanDataset('/Users/zhaomingming/data_sets/pennFudanPed', get_transform(train=False))
...
def val(epoch):
    ...
    for iter, batch in enumerate(val_loader):
        ...
        #    inputs_tmp = Variable(batch[0][0])
        #    inputs_tmp1=cv2.resize(inputs_tmp.permute([1,2,0]).numpy(),(160,160))
        #    inputs_tmp2=torch.tensor(inputs_tmp1).permute([2,0,1])
        #inputs = torch.unsqueeze(inputs_tmp2,0)
        ...
        #masks2=(masks1>0).float()
        #masks3=torch.tensor(cv2.resize(masks2.numpy(),(160,160)))
        #target = torch.unsqueeze(masks3,0).numpy()
        #target = batch['l'].cpu().numpy().reshape(N, h, w)
